 ...
 * Now includes dual-mode: Preview (read-only) and Edit (simple editor)
 */
...
import { EmailContentMetrics } from "@/components/email-content-metrics";
import {
  extractBodyContent,
  getActiveeCampaignInstructions,
} from "@/lib/html-body-extractor";
...
export function EmailPreviewPanel({
  ...
  const fetchRenderedEmail = useCallback(async () => {
    ...
    try {
      ...
      // Extract body-only HTML for ActiveCampaign integration
      const bodyContent = extractBodyContent(renderedHtml);
      setBodyOnlyHtml(bodyContent);

      // Initialize edited content with body content for edit mode
  ...
  const copyContent = async () => {
    ...
    // Determine what to copy based on view mode
    if (viewMode === "text") {
      content = plainTextContent;
    } else if (viewMode === "source") {
      content = bodyOnlyHtml; // Copy body-only HTML in source view
    } else if (viewMode === "edit") {
      content = editedContent; // Copy edited content
    } else {
      content = htmlContent; // Copy full HTML in preview mode
    }
  ...
  const toggleEditMode = () => {
    if (!isEditMode) {
      // Entering edit mode - set to edit view
      setViewMode("edit");
      setIsEditMode(true);
    } else {
      // Exiting edit mode - return to HTML preview
      setViewMode("html");
      setIsEditMode(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col gap-2 px-4 py-3 border-b bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            <Button
              ...
              className={cn(
                "h-9",
                isEditMode && "bg-purple-600 hover:bg-purple-700"
              )}
            >
              {isEditMode ? (
                <>
          ...
          {/* Secondary: View Mode Tabs and Actions */}
          <div className="flex items-center justify-between">
            {/* View Mode Tabs */}
            <div className="flex items-center gap-1 bg-white rounded-lg border p-1">
              <button
        ...
        {/* Content Area */}
        <div className="flex-1 overflow-hidden bg-gray-100 flex gap-4 p-4">
          ...
          <div className="flex-1 overflow-auto">
            {isLoading ? (
              ...
              >
                ...
                {viewMode === "source" && (
                  <div className="h-full overflow-auto p-4">
                    {/* Instructions for ActiveCampaign Integration */}
                    ...
                    <div className="bg-gray-900 rounded-md p-4">
                      <pre className="whitespace-pre-wrap font-mono text-sm text-green-400">
                        {bodyOnlyHtml || "No HTML body content available"}
                      </pre>
                    </div>
                  </div>
                )}
                  ...
                  <div className="h-full overflow-auto p-4">
                    <div className="mb-3 p-2 bg-purple-50 border border-purple-200 rounded-md">
                      <p className="text-xs text-purple-700 flex items-center gap-1">
                        ...
                        Modo de edición - Los cambios se guardan automáticamente
                      </p>
                    </div>
                    <textarea
                      value={editedContent}
                      onChange={(e) => handleEditChange(e.target.value)}
                      className="w-full h-[calc(100%-60px)] p-4 border border-gray-300 rounded-md font-mono text-sm resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                      placeholder="Edita el contenido del email aquí..."
                      spellCheck={false}
                    />
                  </div>
                )}
          ...
          {/* Right Sidebar - Email Content Metrics */}
